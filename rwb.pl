  ...
  my (@str,$error) = Cycles();
    if (!$error) {
      ...
      foreach my $r (@str) { 
        if ($r!=""){
          $out.="\n";
          $out.=join("\n",map {defined($_) ? "<option value=$_>$_</option>" : "<option value=null>null</option>"} @{$r});
        }
...
#
# Generate a table of election cycles
#
# ($table|$raw,$error) = Opinions(latne,longne,latsw,longsw,cycle,format)
# $error false on success, error string on failure
#
sub Cycles {
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select unique cycle from cs339.candidate_master",undef);
  };
